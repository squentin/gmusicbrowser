# This code is copy from (C) 2005-2008 Quentin Sculo <squentin@free.fr>,
# changed and rearanged by <morenojee@gmail.com>
...
my $ON;
my $description = 'Unnamed custom command';
my %menuentry=
(tocmd_1 =>
 {	label => sub { ($::Options{OPT.'tocmd_label_1'} || _$description) },
	code => \&RunCommand_1,
	test => sub {my $c=$::Options{OPT.'tocmd_cmd_1'}; defined $c && $c ne '';},
	notempty => 'IDs',
 },
tocmd_2 =>
 {	label => sub { ($::Options{OPT.'tocmd_label_2'} || _$description) },
	code => \&RunCommand_2,
	test => sub {my $c=$::Options{OPT.'tocmd_cmd_2'}; defined $c && $c ne '';},
	notempty => 'IDs',
 },
tocmd_3 =>
 {	label => sub { ($::Options{OPT.'tocmd_label_3'} || _$description) },
	code => \&RunCommand_3,
	test => sub {my $c=$::Options{OPT.'tocmd_cmd_3'}; defined $c && $c ne '';},
	notempty => 'IDs',
 },
 tocmd_4 =>
 {	label => sub { ($::Options{OPT.'tocmd_label_4'} || _$description) },
	code => \&RunCommand_4,
	test => sub {my $c=$::Options{OPT.'tocmd_cmd_4'}; defined $c && $c ne '';},
	notempty => 'IDs',
 },
# tocmd_5 =>
# {	label => sub { ($::Options{OPT.'tocmd_label_5'} || _$description) },
#	code => \&RunCommand_5,
...
{	my $vbox=Gtk2::VBox->new(::FALSE, 2);
	...
	# CMD4
	my $entry7=::NewPrefEntry(OPT.'tocmd_label_4',_"Menu entry name", sizeg1=> $sg1,tip => _("Name under which the command will appear in the menu"));
	my $entry8=::NewPrefEntry(OPT.'tocmd_cmd_4',_"System command :", sizeg1=> $sg1,
		tip =>  _("These fields can be used :")."\n".::MakeReplaceText('ftalydnAY')."\n".
			_("In this case one command by file will be run\n\n").
			_('Or you can use the field $files which will be replaced by the list of files, and only one command will be run'));
	my $check4=::NewPrefCheckButton(OPT.'tocmd_4',_"Command 4", cb=>\&updatemenu, widget=> ::Vpack($entry7,$entry8));
	$vbox->pack_start($_,::FALSE,::FALSE,2) for $check4;

#	# CMD5
#	my $entry9=::NewPrefEntry(OPT.'tocmd_label_5',_"Menu entry name", sizeg1=> $sg1,tip => _("Name under which the command will appear in the menu"));
#	my $entry10=::NewPrefEntry(OPT.'tocmd_cmd_5',_"System command :", sizeg1=> $sg1,
#		tip =>  _("These fields can be used :")."\n".::MakeReplaceText('ftalydnAY')."\n".
#			_("In this case one command by file will be run\n\n").
#			_('Or you can use the field $files which will be replaced by the list of files, and only one command will be run'));
#	my $check5=::NewPrefCheckButton(OPT.'tocmd_5',_"Command 5", cb=>\&updatemenu, widget=> ::Vpack($entry9,$entry10) );
