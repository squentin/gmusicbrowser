...
{	my $vbox=Gtk2::VBox->new(::FALSE, 2);
	...
	my $entry1=::NewPrefFileEntry(OPT.'path',_"Player mounted on :",'folder',undef,undef,$sg1,$sg2);
	my $entry2=::NewPrefEntry(OPT.'folderformat',_"Folder format :",undef,$sg1,$sg2,undef,_("These fields can be used :")."\n".::MakeReplaceText('talydnAY'));
	my $entry3=::NewPrefEntry(OPT.'filenameformat',_"Filename format :",undef,$sg1,$sg2,undef,_("These fields can be used :")."\n".::MakeReplaceText('talydnAYo'));
	my $check1=::NewPrefCheckButton(OPT.'topath',_"Copy to mounted portable player",sub { updatemenu(); },undef, ::Vpack($entry1,$entry2,$entry3) );

	my $entry4=::NewPrefEntry(OPT.'tocmd_label',_"Menu entry name",undef,$sg1,$sg2,undef,_("Name under which the command will appear in the menu"));
	my $entry5=::NewPrefEntry(OPT.'tocmd_cmd',_"System command :",undef,$sg1,$sg2,undef,_("These fields can be used :")."\n".::MakeReplaceText('ftalydnAY')."\n"._("In this case one command by file will be run\n\n".'Or you can use the field $files which will be replaced by the list of files, and only one command will be run'));
	my $check2=::NewPrefCheckButton(OPT.'tocmd',_"Execute custom command on selected files",sub { updatemenu(); },undef, ::Vpack($entry4,$entry5) );

	my $check3=::NewPrefCheckButton(OPT.'tom3u',_"Export to .m3u file",sub { updatemenu(); });
	my $check4=::NewPrefCheckButton(OPT.'toCSV',_"Export song properties to a .csv file",sub { updatemenu(); });
...
sub ToM3U
{	my $IDs=$_[0]{IDs} || $_[0]{filter}->filter;
	my $file=::ChooseSaveFile(undef,_"Write filenames to ...",$::Songs[ $IDs->[0] ][::SONG_PATH],'list.m3u');
	return unless defined $file;
	open my$fh,'>',$file;
	for my $ID (@$IDs)
	 { print $fh $::Songs[$ID][::SONG_PATH].::SLASH.$::Songs[$ID][::SONG_FILE]."\n"; }
	close $fh;
}
...
{	my $IDs=$_[0]{IDs} || $_[0]{filter}->filter;
	...
	return unless defined $file;
	my @fields=(::SONG_TITLE,::SONG_ARTIST,::SONG_ALBUM,::SONG_DATE,::SONG_COMMENT,::SONG_TRACK,::SONG_DISC,::SONG_LENGTH,::SONG_SIZE,::SONG_RATING,::SONG_MODIF,::SONG_ADDED,::SONG_LASTPLAY,::SONG_NBPLAY,::SONG_LASTSKIP,::SONG_NBSKIP,::SONG_BITRATE,::SONG_FORMAT,::SONG_CHANNELS,::SONG_SAMPRATE,::SONG_GENRE,::SONG_LABELS);
	open my$fh,'>:utf8',$file;
	unless ($::Options{OPT.'toCSV_notitlerow'}) #print a title row
	{	print $fh join(',',map $::TagProp[$_][0], ::SONG_UFILE,::SONG_UPATH,@fields)."\n";
	}
	no warnings 'uninitialized';
	for my $ID (@$IDs)
	{	my $song=$::Songs[$ID];
		my @val;
		push @val, @$song[::SONG_FILE,::SONG_PATH,@fields];
		s/\x00$//g , s/\x00/\t/g for @val; #for genres and labels (\x00-separated)
		s/"/""/g for @val;
