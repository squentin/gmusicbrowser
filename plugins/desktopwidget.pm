...
{	my $key=shift;
	...
	my $textcolor= Gtk3::ColorButton->new_with_rgba( Gtk3::Gdk::RGBA::parse($opt->{DefaultFontColor}) );
	...
	my $adjw= Gtk3::Adjustment->new($opt->{w}, 1, ::max(2000,$opt->{w}, $screen->get_width), 10,50,0);
	my $adjh= Gtk3::Adjustment->new($opt->{h}, 1, ::max(2000,$opt->{h}, $screen->get_height),10,50,0);
