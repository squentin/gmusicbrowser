	...
	{	_		=> '____[#ID#]',
		...
		update	=> 'my @list= split /$::re_artist/o, ##mainfield#->get#;
			...
			#_# =	@ids==1 ? $ids[0] :
				@ids==0 ? 0 :
				(___group{join(" ",map sprintf("%x",$_),@ids)}||= \@ids);',
		'filter:m'	=> '_name .=~. m"#VAL#"',		'filter_prep:m' => \&Filter::QuoteRegEx,
		'filter:mi'	=> '_name .=~. m"#VAL#"i',		'filter_prep:mi'=> \&Filter::QuoteRegEx,
		'filter:s'	=> 'index( lc(_name),"#VAL#") .!=. -1',	'filter_prep:s' => sub { quotemeta lc $_[0]},
...
#warn Songs::Code('album',"get_picture",GID=>'$_[0]')
	...
	{	parent		=> 'generic',
		...
		diff		=> '#_# ne #VAL#',
		s_sort		=> '#_#',
		'filter:m'	=> '#_# .=~. m"#VAL#"',			'filter_prep:m'	=> \&Filter::QuoteRegEx,
		'filter:mi'	=> '#_# .=~. m"#VAL#"i',		'filter_prep:mi'=> \&Filter::QuoteRegEx,
		'filter:s'	=> 'index( lc(#_#),"#VAL#") .!=. -1',	'filter_prep:s'	=> sub {quotemeta lc $_[0]},
		'filter:e'	=> '#_# .eq. "#VAL#"',
		hash		=> '#_#',
		group		=> '#_# ne',
...
#	},
	...
	{	_		=> 'vec(____,#ID#,#bits#)',
		...
		'filter:m'	=> '#_name#[#_#] .=~. m"#VAL#"',			'filter_prep:m'	=> \&Filter::QuoteRegEx,
		'filter:mi'	=> '#_name#[#_#] .=~. m"#VAL#"i',			'filter_prep:mi'=> \&Filter::QuoteRegEx,
...
#		hash		=> '#_name#[#_#]',
	...
	{	parent	=> 'integer',
		...
		'filter:e'	=> '#_# .==. #VAL#',		'filter_prep:e' =>  sub { $_[0] eq "" ? 255 : quotemeta $_[0]},
		'filter:>'	=> '#_default# .>. #VAL#',
		'filter:<'	=> '#_default# .<. #VAL#',
...
sub FieldName
{	my $f=$_[0];
	return $Def{$f} && exists $Def{$f}{name} ? $Def{$f}{name} : _"Unknown field";
}
sub MainField
{	my $f=$_[0];
