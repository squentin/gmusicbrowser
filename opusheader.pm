...
{ 
    ...
    {	
	    ...
	    warn "opusheader.pm : using Digest::CRC\n" if $::debug;
    };

    if ($@)
    { 
        warn "opusheader.pm : Digest::CRC not found, using slow pure-perl replacement.\n" if $::debug;
        @crc_lookup=
...
# The only part of Ogg that's actually formally defined lol (massive migraine ensues)
