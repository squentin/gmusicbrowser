...
{	my ($self,$boxes,$opt2)=@_;
	...
	{	$nb->append_page( $widgets->{$name}, Gtk2::Label->new($name) );
	}
	warn "layout error: (@noparentboxes) have no parent -> can't find toplevel box\n";
	return ::Vpack(Gtk2::Label->new_with_format('<big><b>%s</b></big>',"Layout error: these boxes have no parent, don't know which one is toplevel:"),$nb);
}
...
{	my ($self,$event,$after)=@_;
	...
	return 0 if !$after && $focused && ($focused->isa('Gtk2::Entry') || $focused->isa('Gtk2::TextView') || $focused->isa('Gtk2::SpinButton'));
	my $mod;
